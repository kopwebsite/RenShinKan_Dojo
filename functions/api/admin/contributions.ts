...
async function ensureCurrentPeriod(db: D1Database, month: string, requestId: string) {
  ...
  const statements: D1PreparedStatement[] = [
    ...
    db.prepare(`INSERT OR IGNORE INTO contribution_period_students (
      ...
      current_belt, 1, ? FROM students WHERE active = 1 AND profile_status = 'approved'`)
      .bind(month, now),
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    if (month === currentBangkokMonthKey()) await ensureCurrentPeriod(db, month, requestId);
    ...
    const rows = (await db.prepare(`SELECT
        ...
        c.submitted_at, c.internal_note
      FROM contribution_period_students r
      LEFT JOIN monthly_contributions c ON c.student_id = r.student_id AND c.month_key = r.month_key
      ...
      .bind(month, ...studentIds).all<{
        ...
        contribution_id: string | null; current_status: string; submitted_at: string | null; internal_note: string | null;
      }>()).results || [];
    ...
    for (const row of rows) {
      ...
      const paidBy = newStatus === "paid" ? "primary_admin" : null;
      statements.push(
        ...
        ON CONFLICT(student_id, month_key) DO UPDATE SET status = excluded.status,
          ...
          .bind(contributionId, row.student_id, month, row.student_name_snapshot, row.student_public_id_snapshot,
            newStatus, submittedAt, paidAt, paidBy, now, note || row.internal_note || "", now, now),
        db.prepare(`INSERT INTO contribution_status_history (
          ...
          .bind(crypto.randomUUID(), contributionId, row.current_status, newStatus, bulkOperationId, requestId, note || null, now),
        auditStatement(db, {
          actorType: "administrator", actorIdentifier: "primary_admin",
          ...
          previousValues: { status: row.current_status }, newValues: { status: newStatus, paidAt },
