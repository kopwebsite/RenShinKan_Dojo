...
import { DEFAULT_DOJO_ID, requireStudentDb, type StudentEnv } from "../../_lib/studentRecords";
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  // A badge counts work an administrator can actually open. The profile,
  // training, examination, and monthly contribution screens all hide archived
  // students, so their badges must hide them too or the numbers disagree. The
  // AAT and payslip screens do list archived students, so those two badges
  // deliberately keep counting them.
  const notArchived = "s.deleted_at IS NULL AND s.archived_at IS NULL";
  const countsResult = await db.prepare(`SELECT
    (SELECT COUNT(*) FROM students s
      WHERE ${notArchived} AND s.profile_status = 'pending_admin_approval' ${scope}) AS pending_profiles,
    (SELECT COUNT(*) FROM examination_applications ea
      JOIN examination_cycles ec ON ec.id = ea.cycle_id AND ec.status = 'active'
      JOIN students s ON s.id = ea.student_id
      WHERE ${notArchived} AND ea.status = 'application_submitted' AND ea.payment_status = 'payment_pending' ${scope}) AS pending_exams,
    ...
    (SELECT COUNT(*) FROM training_hour_requests r JOIN students s ON s.id = r.student_id
      WHERE ${notArchived} AND r.status = 'pending' ${scope}) AS pending_hours,
    ${superAdmin ? `(SELECT COUNT(*) FROM monthly_contributions c JOIN students s ON s.id = c.student_id
      WHERE ${notArchived} AND s.dojo_id = '${DEFAULT_DOJO_ID}' AND c.status = 'awaiting_payment')` : "0"} AS pending_monthly_contributions,
