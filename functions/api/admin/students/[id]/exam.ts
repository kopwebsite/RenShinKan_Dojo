...
import { audit, rankColor, requireStudentDb, type StudentEnv } from "../../../../_lib/studentRecords";
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  const db = requireStudentDb(env); const examId = crypto.randomUUID(); const now = new Date().toISOString(); const color = rankColor(belt);
