...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  try {
    const body = await request.json<{ hours?: unknown; location?: unknown; entryDate?: unknown }>();
    const hours = Number(body.hours);
    const entryDate = typeof body.entryDate === "string"
      ? body.entryDate.normalize("NFKC").trim()
    ...
    if (!isCanonicalDate(entryDate)) return jsonResponse({ error: "Training date must use YYYY-MM-DD." }, 400);
    ...
    await db.batch([
      db.prepare(`INSERT INTO training_hours
        ...
        .bind(recordId, studentId, entryDate, hours, location || null, now),
      ...
      auditStatement(db, {
        ...
        studentId, previousValues: { totalHours: previousTotal }, newValues: { hoursAdded: hours, entryDate, location: location || null, totalHours: newTotal },
        source: "admin_student_edit", requestId, summary: `Added ${hours} training hours for ${entryDate}${location ? ` at ${location}` : ""} (${previousTotal} → ${newTotal})`, createdAt: now,
      }),
