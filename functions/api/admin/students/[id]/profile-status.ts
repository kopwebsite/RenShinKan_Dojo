...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  try {
    ...
    const existing = await db.prepare("SELECT id, public_student_id, display_name, profile_status, pending_profile_image_key, profile_image_url FROM students WHERE id = ? LIMIT 1")
      .bind(id).first<{ id: string; public_student_id: string; display_name: string; profile_status: string; pending_profile_image_key: string | null; profile_image_url: string | null }>();
