...
function validProfileUrl(value: unknown) {
  ...
  return /^\/uploads\/student-profiles\/\d{4}\/\d{2}\/[a-f0-9-]{36}\.webp$/i.test(
    url,
  )
    ? url
    : undefined;
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    const dojoIds = (
      ...
      .filter(
        (value, index, values) =>
          /^[a-z0-9-]{1,80}$/i.test(value) && values.indexOf(value) === index,
      )
      .slice(0, 20);
    ...
    const totalExpression =
      "COALESCE((SELECT SUM(verified_hours) FROM training_hours h WHERE h.student_id = s.id), 0) + s.training_hours_adjustment";
    const applicationStatusExpression =
      "COALESCE((SELECT ea.status FROM examination_applications ea JOIN examination_cycles ec ON ec.id = ea.cycle_id AND ec.status = 'active' WHERE ea.student_id = s.id AND ea.status <> 'archived' ORDER BY ea.submitted_at DESC LIMIT 1), 'none')";
    const paymentStatusExpression =
      "COALESCE((SELECT ea.payment_status FROM examination_applications ea JOIN examination_cycles ec ON ec.id = ea.cycle_id AND ec.status = 'active' WHERE ea.student_id = s.id AND ea.status <> 'archived' ORDER BY ea.submitted_at DESC LIMIT 1), 'not_applicable')";
    ...
    if (profileStatus) {
      conditions.push("s.profile_status = ?");
      bindings.push(profileStatus);
    } else if (excludePending)
      conditions.push("s.profile_status <> 'pending_admin_approval'");
    if (examinationStatus) {
      conditions.push(`${applicationStatusExpression} = ?`);
      bindings.push(examinationStatus);
    }
    if (paymentStatus) {
      conditions.push(`${paymentStatusExpression} = ?`);
      bindings.push(paymentStatus);
    }
    if (hoursStatus === "pending")
      conditions.push(
        "EXISTS (SELECT 1 FROM training_hour_requests phr WHERE phr.student_id = s.id AND phr.status = 'pending')",
      );
    if (
      aatStatus === "payment_required" ||
      aatStatus === "new" ||
      aatStatus === "unpaid"
    )
      conditions.push("(s.aat_number IS NULL OR s.aat_last_paid_date IS NULL)");
    else if (aatStatus === "expired")
      conditions.push(
        "s.aat_number IS NOT NULL AND s.aat_last_paid_date IS NOT NULL AND date(s.aat_last_paid_date, '+1 year') < date('now')",
      );
    else if (aatStatus === "current")
      conditions.push(
        "s.aat_number IS NOT NULL AND s.aat_last_paid_date IS NOT NULL AND date(s.aat_last_paid_date, '+1 year') >= date('now')",
      );
    ...
    const [countResult, rowsResult, summaryResult] = await db.batch([
      ...
      db
        .prepare(
          `SELECT s.id, s.public_student_id, s.display_name, s.english_name, s.thai_name, s.account_created_date, s.dojo_joined_date, s.current_belt, s.profile_image_url, s.active, s.archived_at,
    ...
    return jsonResponse(
      {
        ...
        ranks: RANKS,
        suggestedStudentId: await suggestedStudentId(
          db,
          isRenShinKanSuperAdmin(session)
            ? session.selectedDojoId || DEFAULT_DOJO_ID
            : session.selectedDojoId || "__none__",
        ),
        dojos:
          (
            await db
    ...
    operationalEvent("error", "student_list_query_failed", "database_failure", {
      request,
      env,
      status: 500,
      adminAccountId: session.accountId,
      dojoScope: session.selectedDojoId,
    });
    return jsonResponse(
      {
        error:
          "The student list could not be loaded. Existing results were not replaced.",
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    const displayName = String(body.englishName || body.displayName || "")
      .normalize("NFKC")
      .trim()
      .replace(/\s+/g, " ");
    const thaiName =
      String(body.thaiName || "")
        .normalize("NFKC")
        .trim()
        .replace(/\s+/g, " ")
        .slice(0, 120) || null;
    ...
    if (!displayName || displayName.length > 120)
      return jsonResponse(
        { error: "Enter an English name of 120 characters or fewer." },
        400,
      );
    if (!currentBelt)
      return jsonResponse(
        { error: "Choose a valid rank from the official progression." },
        400,
      );
    const dojo = await activeDojo(requireStudentDb(env), dojoId);
    if (!dojo) return jsonResponse({ error: "Choose an active dojo." }, 400);
    if (!canAccessDojo(session, dojo.id))
      return jsonResponse(
        { error: "You do not have access to that dojo." },
        403,
      );
    if (adminNotes.length > 5_000)
      return jsonResponse(
        { error: "The administrator note must be 5,000 characters or fewer." },
        400,
      );
    if (
      !Number.isFinite(currentTrainingHours) ||
      currentTrainingHours < 0 ||
      currentTrainingHours > 1_000_000
    )
      return jsonResponse(
        { error: "Current training hours must be zero or a positive number." },
        400,
      );
    if (profileImageUrl === undefined)
      return jsonResponse(
        { error: "The profile image location is invalid." },
        400,
      );

    const db = requireStudentDb(env);
    const aatNumber =
      String(body.aatNumber || "")
        .normalize("NFKC")
        .trim()
        .slice(0, 40) || null;
    const aatLastPaidDate = isCanonicalDate(body.aatLastPaidDate)
      ? body.aatLastPaidDate
      : null;
    ...
    const dojoJoinedDate = isCanonicalDate(body.dojoJoinedDate)
      ? body.dojoJoinedDate
      : accountCreatedDate;
    const shareFields = JSON.stringify(
      body.shareFields && typeof body.shareFields === "object"
        ? body.shareFields
        : DEFAULT_SHARE_FIELDS,
    );
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
      ...
      try {
        const cycle = await db
          .prepare(
            "SELECT id FROM examination_cycles WHERE status = 'active' ORDER BY created_at DESC LIMIT 1",
          )
          .first<{ id: string }>();
        const currentMonth = currentBangkokMonthKey();
        const contributionPeriod = await db
          .prepare(
        ...
        const statements = [
          ...
          account_created_date, dojo_joined_date, created_at, updated_at
            ...
            .bind(
              ...
              thaiName,
              currentBelt,
              rankColor(currentBelt),
              profileImageUrl,
              body.profileImageConsent ? 1 : 0,
              body.guardianConsent ? 1 : 0,
              body.publicVisible === false ? 0 : 1,
              shareFields,
              dojo.official_name,
              adminNotes,
        ...
        if (aatLastPaidDate) {
          ...
          statements.push(
            db
              .prepare(
                `INSERT INTO aat_membership_payments (id, student_id, dojo_id, payment_date,
              ...
              .bind(
                ...
                dojo.id,
                aatLastPaidDate,
                addOneCalendarYear(aatLastPaidDate),
                session.adminName,
                isRenShinKanSuperAdmin(session) ? "central" : "dojo",
                session.selectedDojoId,
                now,
              ),
            db
            ...
            db
              .prepare(
                `INSERT INTO payment_history (id, payment_id, previous_status, new_status, changed_by, notes, created_at)
        ...
        if (manualStudentId) {
          const sequenceStatement = syncStudentIdSequenceStatement(
            db,
            dojo.id,
            dojo.code,
            studentId,
        ...
        statements.push(
          auditStatement(db, {
            ...
            newValues: {
              ...
              currentTrainingHours,
              dojoId: dojo.id,
              dojoName: dojo.official_name,
              aatNumber,
              aatLastPaidDate,
              profileImageUrl,
              profileImageConsent: Boolean(
                body.profileImageConsent && profileImageUrl,
              ),
              active: true,
            },
            studentPublicId: studentId,
            studentNameSnapshot: displayName,
            source: "admin_students",
            requestId,
            summary: `Created student record ${studentId}`,
            createdAt: now,
          }),
        );
    ...
    return jsonResponse(
      {
        error: "A unique Student ID could not be allocated. Please try again.",
      },
      409,
    );
  } catch (error) {
