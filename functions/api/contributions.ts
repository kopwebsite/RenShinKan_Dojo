...
export const onRequestPost: PagesFunction<StudentEnv> = async ({ request, env }) => {
  ...
  try {
    ...
    if (contributionType === "aat_annual") {
      ...
      const response = { ok: true, contributionId: paymentId, month, status: "awaiting_payment", contributionType };
      await db.batch([
    ...
    const response = { ok: true, contributionId, month, status: "awaiting_payment", contributionType };
