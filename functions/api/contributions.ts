...
function aatReminder(student: StudentRow) {
  ...
  return {
    kind: "aat" as const,
    state: membership.state === "expired" ? "overdue" as const
      : membership.state === "expiring" ? "due_soon" as const
        : membership.state === "current" ? "current" as const : "not_recorded" as const,
...
async function lookupStudent(
  ...
  const student = await db.prepare(`SELECT s.id, s.public_student_id, s.display_name, s.current_belt,
    ...
    .bind(publicStudentId, dojoId).first<StudentRow>();
  return student && namesLikelyMatch(submittedName, student.display_name) ? student : null;
}
...
export const onRequestPost: PagesFunction<StudentEnv> = async ({ request, env }) => {
  ...
  try {
    ...
    const contributionType = body.contributionType === "aat_annual" || body.contributionType === "renshinkan_monthly"
      ? body.contributionType : "";
    const rawStudents = Array.isArray(body.students) ? body.students : [{
      studentId: body.studentId, studentName: body.studentName, dojoId: body.dojoId,
    }];
    const submittedStudents = rawStudents.map((entry) => {
      ...
      return {
        ...
        dojoId: contributionType === "renshinkan_monthly" ? DEFAULT_DOJO_ID : clean(item.dojoId, 80),
      };
    });
    if (!contributionType || !isMonthKey(month) || month !== currentMonth) {
      return jsonResponse({ error: "Choose a contribution type for the current month." }, 400, headers);
    }
    if (submittedStudents.length < 1 || submittedStudents.length > MAX_CONTRIBUTION_STUDENTS
      || submittedStudents.some((student) => !student.publicStudentId || !student.submittedName || !student.dojoId)) {
      return jsonResponse({ error: `Add between 1 and ${MAX_CONTRIBUTION_STUDENTS} students with a dojo, Student ID, and name for each person.` }, 400, headers);
    }
    if (new Set(submittedStudents.map((student) => student.publicStudentId)).size !== submittedStudents.length) {
    ...
    for (const submitted of submittedStudents) {
      const student = await lookupStudent(db, submitted.publicStudentId, submitted.dojoId, submitted.submittedName);
      if (!student) {
        return jsonResponse({ error: `We could not verify ${submitted.publicStudentId} with that dojo and name. Check the row or ask a sensei for help.` }, 404, headers);
      }
    ...
    const statements: D1PreparedStatement[] = [
      db.prepare(`INSERT INTO payment_requests
        ...
        .bind(paymentRequestId, contributionType, new Set(students.map((student) => student.dojo_id)).size === 1 ? students[0].dojo_id : null,
          totalAmount, requestId, now, now),
    ];
    ...
    if (contributionType === "aat_annual") {
      ...
      for (const student of students) {
        ...
        if (pending) return jsonResponse({ error: `${student.public_student_id} already has an AAT contribution awaiting payment confirmation.` }, 409, headers);
        ...
        statements.push(
          ...
          auditStatement(db, {
            actorType: "student", actorIdentifier: students[0].id,
      ...
      const dojoName = new Set(students.map((student) => student.dojo_id)).size === 1
        ? students[0].dojo_name : "Multiple participating dojos";
      const response = {
        ok: true, contributionId: paymentRequestId, month, status: "awaiting_payment",
        contributionType, dojoName, reminder: coveredStudents[0].reminder,
        coveredStudents, studentCount: students.length, unitAmount, totalAmount,
        proofId: proof.proofId, uploadToken: proof.uploadToken,
      };
      statements.push(
        proof.statement,
        db.prepare("INSERT INTO mutation_requests (request_id, actor_type, action, response_json, created_at) VALUES (?, 'student', 'aat_contribution_form_submitted', ?, ?)")
    ...
    statements.push(db.prepare(`INSERT OR IGNORE INTO contribution_periods
    ...
    for (const student of students) {
      ...
      if (duplicate && duplicate.status !== "no_submission") {
        return jsonResponse({ error: `${student.public_student_id} already has a contribution request for ${month}. Remove that student before continuing.` }, 409, headers);
      }
      const lastPayment = await db.prepare(`SELECT month_key, paid_at FROM monthly_contributions
        WHERE student_id = ? AND status = 'paid' AND month_key < ? ORDER BY month_key DESC LIMIT 1`)
        .bind(student.id, month).first<{ month_key: string; paid_at: string | null }>();
      const reminder: MonthlyReminder = {
        kind: "monthly",
        state: !lastPayment ? "not_recorded" : lastPayment.month_key === previousMonthKey(month) ? "current" : "check_in",
      ...
      coveredStudents.push({
        contributionId, studentId: student.public_student_id, studentName: student.display_name,
        dojoId: student.dojo_id, dojoName: student.dojo_name, amount: unitAmount,
        reminder, status: "awaiting_payment",
      });
      statements.push(
        ...
        ON CONFLICT(student_id, month_key) DO UPDATE SET
          ...
          .bind(contributionId, student.id, month, student.display_name, student.public_student_id,
            now, now, student.id, now, now, paymentRequestId, unitAmount),
        db.prepare(`INSERT INTO payment_request_items (
          id, payment_request_id, student_id, dojo_id, payment_reference_id, period_key,
          amount_thb, status, last_verified_date, next_due_date, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, 'awaiting_payment', ?, ?, ?, ?)`)
          .bind(crypto.randomUUID(), paymentRequestId, student.id, student.dojo_id, contributionId, month,
            unitAmount, reminder.lastPaidDate, month, now, now),
        db.prepare(`INSERT INTO contribution_status_history (
          ...
          .bind(crypto.randomUUID(), contributionId, duplicate?.status || "no_submission", student.id, requestId, now),
        auditStatement(db, {
          actorType: "student", actorIdentifier: students[0].id,
          action: "monthly_contribution_form_submitted", entityType: "monthly_contribution", entityId: contributionId,
          studentId: student.id, studentPublicId: student.public_student_id, studentNameSnapshot: student.display_name,
          previousValues: { status: duplicate?.status || "no_submission" },
          newValues: { status: "awaiting_payment", month, paymentRequestId, expectedAmount: unitAmount, groupSize: students.length },
          source: "monthly_contribution_form", requestId, contributionMonth: month,
          summary: `Monthly contribution submitted in a ${students.length}-student payment request`, createdAt: now,
        }),
      );
    }
    const response = {
      ok: true, contributionId: paymentRequestId, month, status: "awaiting_payment",
      contributionType, dojoName: students[0].dojo_name, reminder: coveredStudents[0].reminder,
      coveredStudents, studentCount: students.length, unitAmount, totalAmount,
      proofId: proof.proofId, uploadToken: proof.uploadToken,
    };
    statements.push(
    ...
    console.error("Contribution request failed", requestId, error instanceof Error ? error.message : "Unknown error");
    return jsonResponse({
      error: duplicate ? "A contribution request already exists for one of those students." : "The contribution request could not be recorded. Please try again.",
      code: duplicate ? "CONTRIBUTION_DUPLICATE" : "CONTRIBUTION_REQUEST_FAILED",
      requestId,
    }, duplicate ? 409 : 400, headers);
  }
};
