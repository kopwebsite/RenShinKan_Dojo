...
export const onRequestPost: PagesFunction<StudentEnv> = async ({ request, env }) => {
  ...
  try {
    ...
    if (!publicStudentId || !submittedName || !isMonthKey(month)) {
      return jsonResponse({ error: "Enter a valid Student ID, student name, and contribution month." }, 400, headers);
    }
    ...
    const student = await db.prepare(`SELECT id, public_student_id, display_name, current_belt
      FROM students WHERE UPPER(public_student_id) = ? AND active = 1 AND profile_status = 'approved' LIMIT 1`)
      .bind(publicStudentId).first<{ id: string; public_student_id: string; display_name: string; current_belt: string }>();
    ...
    const response = { ok: true, contributionId, month, status: "awaiting_payment" };
