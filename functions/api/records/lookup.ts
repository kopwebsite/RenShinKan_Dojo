...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    const student = await db.prepare(`SELECT s.id, s.public_student_id, s.display_name, s.english_name, s.thai_name,
      ...
      FROM students s LEFT JOIN dojos d ON d.id = s.dojo_id
      WHERE (s.public_student_id = ? OR EXISTS (
        SELECT 1 FROM student_id_aliases a
        WHERE a.student_id = s.id AND a.alias_public_student_id = ?
      )) AND s.active = 1 AND s.public_visible = 1
      AND s.profile_status IN ('pending_admin_approval', 'approved') LIMIT 1`)
      .bind(studentId, studentId).first<StudentRow>();
    ...
    if (share.created) {
      await audit(db, {
        ...
        summary: "Created a compatible public profile QR link",
      });
    }
    return jsonResponse(
