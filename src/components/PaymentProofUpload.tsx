...
export function PaymentProofUpload({ access, paymentLabel, replacement = false, onUploaded }: {
  ...
  async function upload(file?: File) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.error || "The payment proof could not be uploaded.");
      setMessage("Payment proof submitted. A sensei can now review and confirm your payment.");
      onUploaded?.();
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "The payment proof could not be uploaded.");
    } finally {
      setBusy(false);
  ...
  if (message) return <section className="payment-proof-upload payment-proof-upload--complete" aria-live="polite">
    <div className="payment-proof-upload__heading"><CheckCircle2 aria-hidden="true" /><div><p className="eyebrow">Upload complete</p><h4>Payment proof received</h4><p>{message}</p></div></div>
    ...
    <small><ShieldCheck size={14} /> Your payment proof is retained as a private authenticated record.</small>
  </section>;

  return <section className="payment-proof-upload" aria-live="polite">
    <div className="payment-proof-upload__heading"><FileUp aria-hidden="true" /><div><p className="eyebrow">Payment confirmation</p><h4>{replacement ? "Replace payment proof" : "Upload payment proof"}</h4><p>The dojo cannot confirm your {paymentLabel} payment until you upload proof of payment.</p></div></div>
    ...
    <div className="payment-proof-upload__action"><button className="btn-primary" type="button" disabled={busy} onClick={() => input.current?.click()}>
      ...
      {busy ? "Uploading payment proof…" : replacement ? "Choose replacement file" : "Choose payment proof file"}
    </button><small><ShieldCheck size={14} /> JPEG, PNG, WebP, or PDF up to 5 MB · private authenticated access</small></div>
    <p className="payment-proof-upload__fallback"><strong>Cannot upload?</strong> Please send the payment proof directly to a sensei of RenShinKan Dojo.</p>
