...
export function StudentRecordCard({ record }: { record: PublicStudentRecord }) {
  ...
  return <article className="record-sheet">
    <header>{record.profileImage ? <img src={record.profileImage} alt={`${record.displayName} profile`} /> : <div className="record-sheet__monogram" aria-hidden="true">{record.displayName.slice(0, 1)}</div>}<div><p className="folio-mark">{c.verified}</p><h2>{record.displayName}</h2><p>{record.dojoName} · {record.studentId}</p></div><BadgeCheck aria-label="Verified" /></header>
