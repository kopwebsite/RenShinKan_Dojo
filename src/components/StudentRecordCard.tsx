  ...
  return (
    <article className="record-sheet">
      ...
      <dl className="record-sheet__summary">
        <div><dt>{c.currentBelt}</dt><dd><span className={`belt-dot belt-dot--${record.beltColor}`} />{record.currentBelt}</dd></div>
      ...
      <section><h3>{c.exams}</h3>{record.examinations.length ? <ol className="record-exams">{record.examinations.map((exam) => <li key={`${exam.examination_date}-${exam.belt_awarded}`}><time>{date(exam.examination_date)}</time><strong>{exam.belt_awarded}{exam.rank ? ` · ${exam.rank}` : ""}</strong>{exam.examiner ? <span>{c.examiner}: {exam.examiner}</span> : null}{exam.public_notes ? <p>{exam.public_notes}</p> : null}</li>)}</ol> : <p className="marginal-note">{c.noExams}</p>}</section>
