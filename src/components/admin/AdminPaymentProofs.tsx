...
export function AdminPaymentProofs({
  ...
  showAllDojos: boolean;
  report: (message: string, isError?: boolean) => void;
  scope: "exam" | "contributions";
}) {
  ...
  useEffect(() => {
    void load();
  }, [page, query, status, paymentType, scope]);
  ...
  return (
    <section
      className="admin-workspace-section admin-payment-proofs"
      aria-busy={loading}
    >
      <header className="admin-workspace-heading">
        <div>
          <p className="eyebrow">Private payment review</p>
          <h2>
            {scope === "exam"
              ? "Exam payment proofs"
              : "Contribution payment proofs"}
          </h2>
          <p>
            {scope === "exam"
              ? "Review retained examination fee proofs."
              : "Review retained AAT annual and monthly contribution proofs."}{" "}
            Access is authenticated and audited.
          </p>
        </div>
      </header>
      ...
      <div className="admin-summary admin-payment-proof-summary">
        ...
        <div>
          <strong>{Number(data.summary.pending || 0)}</strong>
          <span>Needs review</span>
        </div>
        <div>
          <strong>{Number(data.summary.approved || 0)}</strong>
          <span>Approved</span>
        </div>
        <div>
          <strong>{Number(data.summary.denied || 0)}</strong>
          <span>Denied</span>
        </div>
      </div>

      <form
        className="admin-record-filters admin-payment-proof-filters"
        onSubmit={(event) => {
      ...
      >
        <label className="admin-search-wide">
          Search students
          <div>
            <Search size={17} />
            <input
        ...
        <label>
          ...
          <select
            value={status}
            onChange={(event) => {
              setStatus(event.target.value as ProofStatus | "");
              setPage(1);
              setSelected(new Set());
            }}
          >
        ...
        {scope === "contributions" ? (
          <label>
            ...
            >
              ...
              <option value="renshinkan_monthly">Monthly contribution</option>
            </select>
          </label>
        ) : null}
        {query || status !== "pending_review" || paymentType ? (
          <button
            type="button"
      ...
      {selected.size ? (
        <aside className="admin-bulk-toolbar admin-payment-proof-bulk">
          <strong>
            {selected.size} pending payment proof
            {selected.size === 1 ? "" : "s"} selected on this page
          </strong>
          <button
            className="btn-secondary"
            onClick={() =>
      ...
      <div className="admin-table-scroll">
        <table className="admin-record-table admin-payment-proof-table">
          <thead>
            <tr>
              <th>
                <label className="admin-select-box">
                  <input
                    type="checkbox"
                    aria-label="Select every pending payment proof on this page"
                    checked={allPendingSelected}
                    onChange={(event) =>
                      setSelected(
                        event.target.checked
                          ? new Set(pendingRows.map((proof) => proof.id))
                          : new Set(),
                      )
                    }
                  />
          ...
          <tbody>
            {data.proofs.map((proof) => (
              <tr
                key={proof.id}
                className={selected.has(proof.id) ? "is-selected" : ""}
              >
                <td>
                  {proof.status === "pending_review" ? (
                    <label className="admin-select-box">
                      <input
                        type="checkbox"
                        aria-label={`Select payment proof from ${proof.student_name}`}
                        checked={selected.has(proof.id)}
                        onChange={(event) =>
                          toggle(proof.id, event.target.checked)
                        }
                      />
                      <span aria-hidden="true" />
                    </label>
                ...
                <th>
                  <span className="admin-student-identity">
                    {proof.profile_image_url ? (
                      <img src={proof.profile_image_url} alt="" />
                    ) : (
                      <span aria-hidden="true">
                        <UserRound size={18} />
                      </span>
                    ...
                    <span>
                      {proof.student_name}
                      {proof.covered_student_count > 1 ? (
                        <small> + {proof.covered_student_count - 1} more</small>
                      ) : null}
                    </span>
                  </span>
                </th>
                <td>
                  <code>{proof.public_student_id}</code>
                </td>
                {showAllDojos ? <td>{proof.dojo_name}</td> : null}
                <td>
                  {purposeLabel(proof.payment_type)}
                  {proof.covered_student_count > 1 ? (
                    <small className="admin-table-subline">
                      {proof.covered_student_count} students
                    </small>
                  ) : null}
                </td>
                <td>{formatAdminDate(proof.submitted_at)}</td>
                <td>
                  <ProofStatusBadge value={proof.status} />
                </td>
                <td>
                  <div className="admin-row-actions">
                    <button type="button" onClick={() => setOpened(proof)}>
                      <FileImage size={14} /> View proof
                    </button>
                  </div>
                </td>
              </tr>
            ))}
            {!data.proofs.length && !loading ? (
              <tr>
                <td colSpan={showAllDojos ? 8 : 7}>
                  <div className="admin-empty-inline">
                    <FileImage size={24} />
                    <strong>
                      No submitted payment proofs match these filters.
                    </strong>
                  </div>
                </td>
              </tr>
      ...
      <footer className="admin-pagination">
        <span>
          {data.pagination.total} payment proof
      ...
      {opened ? (
        <div className="admin-confirm-backdrop" role="presentation">
          <section
            className="admin-confirm-dialog admin-payment-proof-dialog"
            role="dialog"
            aria-modal="true"
          ...
          >
            <header>
              <div>
                <p className="eyebrow">{purposeLabel(opened.payment_type)}</p>
                <h2 id="payment-proof-title">{opened.student_name}</h2>
                <p>
                  {opened.public_student_id}
                  {showAllDojos ? ` / ${opened.dojo_name}` : ""}
                </p>
              </div>
              <button
            ...
            <div className="admin-payment-proof-image">
              {opened.content_type === "application/pdf" ? (
                <iframe
                  src={`/api/admin/payment-proofs/${opened.id}`}
                  title={`Payment proof submitted by ${opened.student_name}`}
                />
              ) : (
                <img
                  src={`/api/admin/payment-proofs/${opened.id}`}
                  alt={`Payment proof submitted by ${opened.student_name}`}
                />
              )}
            </div>
            <p className="admin-payment-proof-open">
              <a
                className="btn-secondary"
                href={`/api/admin/payment-proofs/${opened.id}`}
                target="_blank"
            ...
            <dl className="admin-payment-proof-details">
              <div>
                <dt>Payment for</dt>
                <dd>{purposeLabel(opened.payment_type)}</dd>
              </div>
              {opened.covered_student_count > 1 ? (
                <div className="admin-payment-proof-details__wide">
                  <dt>Students covered ({opened.covered_student_count})</dt>
                  <dd>{opened.covered_students}</dd>
                </div>
              ) : null}
              <div>
                <dt>Submitted</dt>
                <dd>{formatAdminDate(opened.submitted_at)}</dd>
              </div>
              <div>
                <dt>Status</dt>
                <dd>
                  <ProofStatusBadge value={opened.status} />
                </dd>
              </div>
              {opened.reviewed_at ? (
                <div>
                  <dt>Reviewed</dt>
                  <dd>
                    {formatAdminDate(opened.reviewed_at)} by{" "}
                    {opened.reviewed_by || "administrator"}
                  </dd>
                </div>
              ) : null}
            </dl>
            <p className="admin-payment-proof-privacy">
              ...
              or share it unless required for the payment review.
            </p>
            <footer>
              <button className="btn-secondary" onClick={() => setOpened(null)}>
                Keep pending
              </button>
              {opened.status === "pending_review" ? (
                <>
                  <button
                    className="btn-secondary is-danger"
                    onClick={() =>
      ...
      {review ? (
        <div
          className="admin-confirm-backdrop admin-payment-review-backdrop"
          role="presentation"
        >
          <section
          ...
          >
            <header>
              <div>
                <p className="eyebrow">Confirm payment review</p>
                <h2 id="payment-review-title">
                  {review.action === "approve"
                    ? "Confirm payment for"
                    : "Reject"}{" "}
                  {review.proofIds.length} payment proof
                  {review.proofIds.length === 1 ? "" : "s"}?
                </h2>
              </div>
              <button
