...
export function AdminAuditPage() {
  ...
  return <section className="container-shell admin-audit-page"><header><div>{selected?.logo_url ? <img className="admin-selected-dojo-logo" src={selected.logo_url} alt="" /> : null}<p className="eyebrow">{selected?.official_name} / ADMIN</p><h1>Audit log</h1><p>{superAdmin ? "All dojo and student changes, with structured before-and-after values retained for investigation." : "Changes to this dojo and its students, with structured before-and-after values retained for investigation."}</p></div><div className="admin-header-actions"><Link className="btn-secondary" to="/admin/students"><ChevronLeft size={16} /> Students</Link>{superAdmin ? <button className="btn-secondary" onClick={() => void session.switchDojo()}><RotateCcw size={16} /> Switch dojo</button> : null}<button className="btn-secondary" onClick={() => void session.logout()}><LogOut size={16} /> Sign out</button></div></header>
    <form className="admin-audit-filters" onSubmit={submit}><label>Search summaries<input value={filters.search} onChange={(event) => setFilters({ ...filters, search: event.target.value })} /></label>{superAdmin ? <label>Dojo<select value={filters.dojoId} onChange={(event) => setFilters({ ...filters, dojoId: event.target.value })}><option value="">All dojos</option>{dojos.map((dojo) => <option key={dojo.id} value={dojo.id}>{dojo.official_name}</option>)}</select></label> : null}<label>Student<input value={filters.student} onChange={(event) => setFilters({ ...filters, student: event.target.value })} placeholder="Name or ID" /></label><label>Administrator<input value={filters.administrator} onChange={(event) => setFilters({ ...filters, administrator: event.target.value })} /></label><label>Actor type<select value={filters.actorType} onChange={(event) => setFilters({ ...filters, actorType: event.target.value })}><option value="">All</option><option value="administrator">Administrator</option><option value="student">Student</option><option value="system">System</option></select></label><label>Action<input value={filters.action} onChange={(event) => setFilters({ ...filters, action: event.target.value })} placeholder="rank_changed" /></label><label>Source<input value={filters.source} onChange={(event) => setFilters({ ...filters, source: event.target.value })} /></label><label>Exam cycle ID<input value={filters.examCycleId} onChange={(event) => setFilters({ ...filters, examCycleId: event.target.value })} /></label><label>Contribution month<input type="month" value={filters.month} onChange={(event) => setFilters({ ...filters, month: event.target.value })} /></label><label>Bulk operation ID<input value={filters.bulkOperationId} onChange={(event) => setFilters({ ...filters, bulkOperationId: event.target.value })} /></label><label>From<input type="date" value={filters.dateFrom} onChange={(event) => setFilters({ ...filters, dateFrom: event.target.value })} /></label><label>To<input type="date" value={filters.dateTo} onChange={(event) => setFilters({ ...filters, dateTo: event.target.value })} /></label><button className="btn-primary"><Search size={16} /> Filter</button></form>
