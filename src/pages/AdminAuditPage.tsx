import { ChevronLeft, ChevronRight, History, LogOut, RotateCcw, Search } from "lucide-react";
...
export function AdminAuditPage() {
  ...
  const [dojos, setDojos] = useState<Array<{ id: string; official_name: string }>>([]);
  ...
  useEffect(() => {
    if (session.admin?.permissionLevel !== "renshinkan_super_admin") return;
  ...
  }, [session.admin?.permissionLevel, page, applied]);
  useEffect(() => { fetch("/api/dojos").then((response) => response.json()).then((body: { dojos?: Array<{ id: string; official_name: string }> }) => setDojos(body.dojos || [])).catch(() => undefined); }, []);
  ...
  if (session.admin.permissionLevel !== "renshinkan_super_admin") return <main className="container-shell py-16"><h1>Access restricted</h1><p>Only the verified RenShinKan administrator may view the global audit history.</p><Link className="btn-secondary mt-5" to="/admin">Back to administration</Link></main>;
  const selected = session.dojos.find((dojo) => dojo.id === session.admin?.selectedDojoId);
  return <section className="container-shell admin-audit-page"><header><div>{selected?.logo_url ? <img className="admin-selected-dojo-logo" src={selected.logo_url} alt="" /> : null}<p className="eyebrow">{selected?.official_name} / ADMIN</p><h1>Audit log</h1><p>Readable summaries with the structured before-and-after values retained for investigation.</p></div><div className="admin-header-actions"><Link className="btn-secondary" to="/admin/students"><ChevronLeft size={16} /> Students</Link><button className="btn-secondary" onClick={() => void session.switchDojo()}><RotateCcw size={16} /> Switch dojo</button><button className="btn-secondary" onClick={() => void session.logout()}><LogOut size={16} /> Sign out</button></div></header>
    <form className="admin-audit-filters" onSubmit={submit}><label>Search summaries<input value={filters.search} onChange={(event) => setFilters({ ...filters, search: event.target.value })} /></label><label>Dojo<select value={filters.dojoId} onChange={(event) => setFilters({ ...filters, dojoId: event.target.value })}><option value="">All dojos</option>{dojos.map((dojo) => <option key={dojo.id} value={dojo.id}>{dojo.official_name}</option>)}</select></label><label>Student<input value={filters.student} onChange={(event) => setFilters({ ...filters, student: event.target.value })} placeholder="Name or ID" /></label><label>Administrator<input value={filters.administrator} onChange={(event) => setFilters({ ...filters, administrator: event.target.value })} /></label><label>Actor type<select value={filters.actorType} onChange={(event) => setFilters({ ...filters, actorType: event.target.value })}><option value="">All</option><option value="administrator">Administrator</option><option value="student">Student</option><option value="system">System</option></select></label><label>Action<input value={filters.action} onChange={(event) => setFilters({ ...filters, action: event.target.value })} placeholder="rank_changed" /></label><label>Source<input value={filters.source} onChange={(event) => setFilters({ ...filters, source: event.target.value })} /></label><label>Exam cycle ID<input value={filters.examCycleId} onChange={(event) => setFilters({ ...filters, examCycleId: event.target.value })} /></label><label>Contribution month<input type="month" value={filters.month} onChange={(event) => setFilters({ ...filters, month: event.target.value })} /></label><label>Bulk operation ID<input value={filters.bulkOperationId} onChange={(event) => setFilters({ ...filters, bulkOperationId: event.target.value })} /></label><label>From<input type="date" value={filters.dateFrom} onChange={(event) => setFilters({ ...filters, dateFrom: event.target.value })} /></label><label>To<input type="date" value={filters.dateTo} onChange={(event) => setFilters({ ...filters, dateTo: event.target.value })} /></label><button className="btn-primary"><Search size={16} /> Filter</button></form>
    {error ? <p className="form-error">{error}</p> : null}<div className="admin-audit-list"><div className="admin-table-meta"><span><History size={15} /> {pagination.total} permanent entries</span></div>{entries.map((entry) => <article key={entry.id}><header><div><strong>{entry.action_summary || entry.action.replace(/_/g, " ")}</strong><span>{new Date(entry.created_at).toLocaleString()}</span></div><span className="admin-status is-neutral">{entry.actor_type}</span></header><p>{entry.student_name ? `${entry.student_name} (${entry.public_student_id}) · ` : ""}{entry.actor_identifier} · {entry.source}</p>{pretty(entry.previous_values) || pretty(entry.new_values) ? <dl><div><dt>Before</dt><dd>{pretty(entry.previous_values) || "—"}</dd></div><div><dt>After</dt><dd>{pretty(entry.new_values) || "—"}</dd></div></dl> : null}{entry.administrator_name ? <p><strong>{entry.administrator_name}</strong>{" / "}{entry.administrator_role || "administrator"}{entry.selected_dojo_id ? ` / ${dojos.find((dojo) => dojo.id === entry.selected_dojo_id)?.official_name || entry.selected_dojo_id}` : ""}{entry.country_code ? ` / ${entry.country_code}` : ""}</p> : null}{entry.ip_address || entry.user_agent ? <details><summary>Request context</summary><p>{entry.ip_address ? `IP ${entry.ip_address}` : "IP unavailable"}{entry.user_agent ? ` / ${entry.user_agent}` : ""}</p></details> : null}<footer><code>Request {entry.request_id}</code>{entry.entity_id ? <code>{entry.entity_type} {entry.entity_id}</code> : null}{entry.bulk_operation_id ? <code>Bulk {entry.bulk_operation_id}</code> : null}{entry.administrator_note ? <span>Note: {entry.administrator_note}</span> : null}</footer></article>)}</div><nav className="admin-pagination"><button className="btn-secondary" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}><ChevronLeft size={16} /> Previous</button><span>Page {pagination.page} of {pagination.totalPages}</span><button className="btn-secondary" disabled={page >= pagination.totalPages} onClick={() => setPage((value) => value + 1)}>Next <ChevronRight size={16} /></button></nav>
  </section>;
}
