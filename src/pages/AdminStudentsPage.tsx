...
import { AdminDojoSelector, AdminLoginFields, type AdminDojo } from "../components/admin/AdminAccess";
...
export function AdminStudentsPage({ mode = "students" }: { mode?: StudentPageMode }) {
  ...
  useEffect(() => { if (authed && admin?.selectedDojoId) void load(page); }, [authed, admin?.selectedDojoId, page, query, rank, examinationStatus, paymentStatus, hoursStatus, dojoFilters, aatStatus, status, sort]);
  useEffect(() => {
  ...
  async function runSelectionAction() {
    ...
    if (selectionAction.type === "reject" && !selectionAction.studentVisibleNote.trim()) return;
    setLoading(true); setError("");
      ...
      try {
        if (selectionAction.type === "approve" || selectionAction.type === "reject") {
          await api(`/api/admin/students/${student.id}/profile-status`, {
            method: "POST",
            body: JSON.stringify({
              action: selectionAction.type,
              studentVisibleNote: selectionAction.studentVisibleNote.trim(),
              internalNote: selectionAction.internalNote.trim(),
            }),
          });
...
function SelectionActionModal({ action, setAction, students, close, confirm, busy }: { action: SelectionAction; setAction: (value: SelectionAction) => void; students: StudentSummary[]; close: () => void; confirm: () => void; busy: boolean }) {
  ...
  const copy = action.type === "approve"
    ...
    : action.type === "reject"
      ? "Only selected profiles still awaiting approval will be denied and kept private. Explain the decision in student-friendly language."
      : action.type === "archive"
  ...
  const valid = count > 0 && (action.type !== "reject" || Boolean(action.studentVisibleNote.trim())) && (action.type !== "delete" || action.confirmationText.trim() === deletePhrase);
  return <div className="admin-confirm-backdrop"><section className="admin-bulk-modal admin-selection-modal" role="alertdialog" aria-modal="true" aria-labelledby="selection-action-title">
    ...
    <form className="admin-bulk-form" onSubmit={(event) => { event.preventDefault(); if (valid) confirm(); }}>
      ...
      {action.type === "approve" || action.type === "reject" ? <>
        <label>Student-visible explanation <small>Shown in each student’s authenticated passport. {action.type === "reject" ? "Required for denial." : "Optional."}</small><textarea value={action.studentVisibleNote} maxLength={2000} onChange={(event) => setAction({ ...action, studentVisibleNote: event.target.value })} required={action.type === "reject"} /></label>
        <label>Private internal note <small>Administrator-only context. Never shown to students or the public. Optional.</small><textarea value={action.internalNote} maxLength={2000} onChange={(event) => setAction({ ...action, internalNote: event.target.value })} /></label>
      </> : null}
