...
export function SharedStudentRecordPage() {
  ...
  return <section className="container-shell shared-record-page"><div className="shared-record-page__tools"><Link to="/student-records" className="text-link"><ArrowLeft size={16} /> {c.lookupLink}</Link><button className="text-link" onClick={share}><Share2 size={16} /> {c.share}</button><button className="text-link" onClick={() => window.print()}><Printer size={16} /> {c.print}</button></div><div className="shared-record-page__layout"><StudentRecordCard record={record} /><aside>{qr ? <img src={qr} alt={c.qrTitle} /> : null}<h2>{c.qrTitle}</h2><p>{c.qrBody}</p><p className="marginal-note">{c.qrNote}</p></aside></div></section>;
}
