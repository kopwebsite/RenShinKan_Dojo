...
export function SharedStudentRecordPage() {
  ...
  const languagePicker = <label className="shared-record-language" htmlFor="shared-record-language"><span>{actions.language}</span><select id="shared-record-language" name="shared-record-language" value={language} onChange={(event) => chooseLanguage(event.target.value as Language)}>{languageOptions.map((option) => <option key={option.code} value={option.code}>{option.nativeLabel}</option>)}</select></label>;
