...
import { CheckCircle2, Clock3, Copy, Download, FileCheck2, Info, LockKeyhole, Printer, QrCode, Search, Share2, ShieldCheck, UserPlus, UserRound } from "lucide-react";
...
function ExamWorkflow() {
  const [draft, setDraft] = useState<ExamDraft>(EMPTY_EXAM); const [stage, setStage] = useState<"form" | "review" | "done">("form"); const [token, setToken] = useState(""); const [reset, setReset] = useState(0); const [error, setError] = useState(""); const [busy, setBusy] = useState(false); const [applicationId, setApplicationId] = useState(""); const [proofAccess, setProofAccess] = useState<PaymentProofAccess | null>(null);
  const onToken = useCallback((value: string) => setToken(value), []);
  ...
  if (stage === "done") return <div className="exam-payment-panel"><AlertMark /><p className="eyebrow">Application {applicationId.slice(0, 8)}</p><h2>Your application is submitted, but you are not finished yet</h2><p>You must still make the examination payment and upload your payslip. The dojo cannot confirm your payment until the payslip is reviewed.</p><div><img src="/images/promptpay-qr.png" alt="Bank payment QR code" /><section><h3>Pay and submit proof</h3><ol><li>Scan this bank-payment QR code and make the examination payment.</li><li>Upload your payslip below.</li><li>Wait for a sensei or administrator to approve the payslip.</li></ol><p><strong>This is the bank-payment QR code.</strong> It is different from your shareable student-profile QR code.</p></section></div>{proofAccess ? <PaymentProofUpload access={proofAccess} paymentLabel="examination" /> : null}</div>;
  ...
  return <form className="student-long-form exam-application-form" onSubmit={review}>
    ...
    <fieldset>
      ...
      <label>Student ID <small>Example: RSK-6901.</small><input value={draft.studentId} onChange={(event) => set("studentId", event.target.value.toUpperCase())} required /></label>
