...
function ExamWorkflow() {
  const [draft, setDraft] = useState<ExamDraft>(EMPTY_EXAM); const [stage, setStage] = useState<"form" | "review" | "done">("form"); const [token, setToken] = useState(""); const [reset, setReset] = useState(0); const [error, setError] = useState(""); const [busy, setBusy] = useState(false); const [applicationId, setApplicationId] = useState("");
  const onToken = useCallback((value: string) => setToken(value), []);
  ...
  return <form className="student-long-form exam-application-form" onSubmit={review}>
    ...
    <fieldset>
      ...
      <label>Student ID <small>Example: RSK-00001.</small><input value={draft.studentId} onChange={(event) => set("studentId", event.target.value.toUpperCase())} required /></label>
