...
describe("multi-dojo authentication and authorization", () => {
  it("requires an administrator name at login and maps distinct server-side password hashes", async () => {
    const secret = "test-session-secret"; const central = await hmac(secret, "central-pass"); const cmu = await hmac(secret, "cmu-pass");
    const env = { SESSION_SECRET: secret, ADMIN_PASSWORD_HASH: central, DOJO_ADMIN_PASSWORD_HASHES: JSON.stringify({ "dojo-cmu": cmu }) };
    expect(await authenticateAdminPassword("central-pass", env)).toEqual({ role: "central", allowedDojoIds: [] });
    expect(await authenticateAdminPassword("cmu-pass", env)).toEqual({ role: "dojo", allowedDojoIds: ["dojo-cmu"] });
  ...
  it("leaves the dojo unselected after central login", async () => {
    ...
    const response = await loginAdmin({
      request: new Request("https://example.test/api/admin/login", {
        ...
        body: JSON.stringify({ adminName: "Central Admin", password: "central-pass" }),
      }),
      env,
    } as never);
    ...
    const parsed = await getAdminSession(new Request("https://example.test/admin", {
      headers: { Cookie: setCookie!.split(";")[0] },
    }), env);
    expect(parsed).toMatchObject({ role: "central", selectedDojoId: null, renshinkanVerified: false });
  });
  ...
  it("signs role, allowed dojos, administrator name, and selection into a tamper-evident cookie", async () => {
    ...
    const cookie = await createSessionCookie(env, { adminName: "Naree", role: "dojo", allowedDojoIds: ["dojo-cmu"], selectedDojoId: "dojo-cmu" });
    const parsed = await getAdminSession(new Request("https://example.test/admin", { headers: { Cookie: cookie.split(";")[0] } }), env);
    expect(parsed).toMatchObject({ adminName: "Naree", role: "dojo", allowedDojoIds: ["dojo-cmu"], selectedDojoId: "dojo-cmu" });
  });
  ...
  it("returns 403 for direct read and write API attempts against another dojo", async () => {
    ...
    const putResponse = await putStudent({ request: new Request("https://example.test/api/admin/students/nu-student", { method: "PUT", headers: { Cookie: cookie.split(";")[0], Origin: "https://example.test", "Content-Type": "application/json" }, body: "{}" }), env, params: { id: "nu-student" } } as never);
...
describe("multi-dojo data model and workflows", () => {
  ...
  it("creates exactly the six official dojo seeds without destructive data reset", () => {
    for (const name of ["Ai Dojo", "Chiang Mai University Aikido Club", "RenShinKan Dojo", "Naresuan University Aikido Club", "All Gym Chiang Mai", "Aikido Mae Hong Son"]) expect(migration).toContain(name);
  ...
  it("requires dojo assignment for admin creation and self-registration, leaving AAT number nullable", () => {
    const admin = file("functions/api/admin/students/index.ts"); const self = file("functions/api/records/profile-requests.ts");
    for (const source of [admin, self]) { expect(source).toContain("activeDojo"); expect(source).toContain("aatNumber"); expect(source).toContain("aatLastPaidDate"); }
    expect(file("src/pages/StudentRecordsPage.tsx")).toContain("Not yet paid or payment date unknown");
  });

  it("calculates NEW, unpaid, current, expiring, and expired AAT states", () => {
    expect(aatMembershipStatus(null, null, new Date("2026-07-21T00:00:00Z"))).toMatchObject({ state: "new", label: "Payment required" });
    expect(aatMembershipStatus("AAT-7", null, new Date("2026-07-21T00:00:00Z")).state).toBe("unpaid");
    expect(aatMembershipStatus(null, "2026-01-01", new Date("2026-07-21T00:00:00Z")).state).toBe("current");
    expect(aatMembershipStatus("AAT-7", "2026-01-01", new Date("2026-07-21T00:00:00Z")).state).toBe("current");
    expect(aatMembershipStatus("AAT-7", "2025-08-01", new Date("2026-07-21T00:00:00Z")).state).toBe("expiring");
    expect(aatMembershipStatus("AAT-7", "2025-01-01", new Date("2026-07-21T00:00:00Z")).state).toBe("expired");
...
describe("examination reports and website editor", () => {
  it("creates a real XLSX package with frozen headers, filters, numeric fees, formulas, and a summary sheet", () => {
    const bytes = buildExamXlsx({ title: "Exam", name: "Exam", rank_category: "Kyu", examination_at: "2026-06-28", venue: "RenShinKan", instructions: "" }, [{ public_student_id: "CMU-00001", student_name: "ทดสอบ Student", dojo_name: "Chiang Mai University Aikido Club", dojo_code: "CMU", aat_number: null, aat_last_paid_date: null, current_rank: "5 Kyu", attempted_rank: "4 Kyu", last_examination_date: null, practice_period: "90 days", grade_given: "", exam_fee: 500, aat_annual_fee: 200, other_fees: 0, total_fee: 700, notes: "", answers_json: '{"age":24}' }], "CMU");
    expect([...bytes.slice(0, 2)]).toEqual([80, 75]); const files = unzipSync(bytes);
    expect(Object.keys(files)).toContain("xl/worksheets/sheet2.xml");
    const sheet = strFromU8(files["xl/worksheets/sheet1.xml"]); expect(sheet).toContain('state="frozen"'); expect(sheet).toContain("<autoFilter"); expect(sheet).toContain("<f>SUM("); expect(sheet).toContain("ทดสอบ Student");
  });

  it("scopes both PDF and Excel on the server and embeds a Unicode Thai font", () => {
    const endpoint = file("functions/api/admin/examinations/export.ts"); const exporter = file("functions/_lib/examExports.ts");
    expect(endpoint).toContain("canAccessDojo"); expect(endpoint).toContain('format === "xlsx"'); expect(endpoint).toContain("NotoSansThai.ttf");
    expect(exporter).toContain("registerFontkit"); expect(exporter).toContain("Page ${pageNumber}"); expect(endpoint).toContain("All Dojos");
  });

  it("renders a valid landscape Unicode PDF with Thai and English student text", async () => {
    const row = { public_student_id: "CMU-00001", student_name: "ทดสอบ Student", dojo_name: "Chiang Mai University Aikido Club", dojo_code: "CMU", aat_number: "AAT-7", aat_last_paid_date: "2026-01-01", current_rank: "5 Kyu", attempted_rank: "4 Kyu", last_examination_date: "2025-11-30", practice_period: "90 days", grade_given: "Pass", exam_fee: 500, aat_annual_fee: 200, other_fees: 0, total_fee: 700, notes: "พร้อม", answers_json: '{"age":24}' };
    const bytes = await buildExamPdf({ title: "การสอบ Exam", name: "Exam", rank_category: "Kyu", examination_at: "2026-06-28", venue: "RenShinKan", instructions: "" }, [row], "All Dojos", new Uint8Array(readFileSync(resolve(root, "public/fonts/NotoSansThai.ttf"))), false);
    expect(new TextDecoder().decode(bytes.slice(0, 8))).toContain("%PDF"); expect(bytes.length).toBeGreaterThan(5_000);
  });

  it("validates multilingual structured content and strips unsafe destinations", () => {
    const content = validateEditableContent({ version: 1, recentEvents: [], examAnnouncement: null, historyMedia: [], onTheMatMedia: [], passedTestStudents: [], sitePages: [{ id: "page-home", route: "/", status: "published", translations: { en: { title: "Home" } }, blocks: [{ id: "block-hero", type: "hero", translations: { en: { title: "Welcome", buttonUrl: "javascript:alert(1)", imageUrl: "https://example.test/image.webp" } } }] }], siteSettings: {} });
    expect(content.sitePages[0].blocks[0].translations.en.buttonUrl).toBe("");
    expect(Object.keys(content.sitePages[0].translations)).toEqual(["en", "th", "ja", "zh-CN"]);
  });

  it("limits the editor to central admins and implements draft, preview, publish, revision, and rollback", () => {
    const api = file("functions/api/admin/site-content.ts"); const ui = file("src/pages/AdminSiteEditorPage.tsx");
    expect(api).toContain("requiresCentralAdmin"); for (const value of ["site_content_drafts", "site_revisions", 'action === "rollback"', "writeEditableContentToStorage"]) expect(api).toContain(value);
    for (const value of ["Unsaved changes", "Draft preview", "desktop", "tablet", "mobile", "Save draft", "Roll back and publish"]) expect(ui).toContain(value);
  });
});
